...
# Prompt user for the path. What is the full path and name of the log file to split?
$sourcefilename = "D:\hadoop\project\texts\input.txt"
 
# Prompt user for the destination folder to create the chunk files. What is the path where you want to extract the content?
$destinationfolderpath = "D:\hadoop\project\texts\"  
...
# Prompt user for the size of the new chunk files. How many lines will be in each new split file?
...
$filenumber = 1
cmd.exe /c "hadoop fs -mkdir /inputlogs"
DO
{
  cmd.exe /c "hadoop fs -put splitlog$filenumber.txt /inputlogs/"
  $filenumber++
} While ($filenumber -le $countall)
